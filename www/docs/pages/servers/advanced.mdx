...
func main() {
    ...
    // Add typed tool
    s.AddTypedTool("calculate", "Perform arithmetic operations", handleCalculateTyped)

    server.ServeStdio(s)
}

func handleCalculateTyped(ctx context.Context, input CalculateInput) (*CalculateOutput, error) {
    var result float64
...
func handleCreateUser(ctx context.Context, input UserCreateInput) (*UserCreateOutput, error) {
    ...
    s := server.NewMCPServer("Session Server", "1.0.0",
        ...
        server.WithHooks(&server.Hooks{
            OnSessionStart: func(sessionID string) {
                ...
                sessionManager.CreateSession(sessionID, "anonymous", []string{"read"})
...
func (m *LoggingMiddleware) ToolMiddleware(next server.ToolHandler) server.ToolHandler {
    return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
        start := time.Now()
        sessionID := server.GetSessionID(ctx)
        
        m.logger.Printf("Tool call started: session=%s tool=%s", sessionID, req.Params.Name)
...
func handleLongRunningTool(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
    sessionID := server.GetSessionID(ctx)
    notifier := server.GetNotifier(ctx)
    
    // Send initial notification
    notifier.SendProgress(sessionID, mcp.ProgressNotification{
        Progress: 0,
        Total:    100,
    ...
    for i := 0; i < 100; i++ {
        ...
        if i%10 == 0 {
            notifier.SendProgress(sessionID, mcp.ProgressNotification{
                Progress: i + 1,
                Total:    100,
                Message:  fmt.Sprintf("Processed %d/100 items", i+1),
            })
        }
    }
    ...
    return mcp.NewToolResultText("Long operation completed"), nil
}
```
...
func main() {
    ...
    s := server.NewMCPServer("Production Server", "1.0.0",
        ...
        server.WithHooks(businessHooks),
        server.WithToolMiddleware(loggingMW.ToolMiddleware),
        server.WithToolMiddleware(rateLimitMW.ToolMiddleware),
        server.WithToolMiddleware(authMW.ToolMiddleware),
        server.WithResourceMiddleware(loggingMW.ResourceMiddleware),
